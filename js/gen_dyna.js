...
for (let categ = 0; categ < total_cat; categ++) {
    for (let i = 0; i < tab_categorie[categ].length; i++) {
        ...
        console.log(identit);
        var quantity = 0;
        var test = new article (identit, tab_categorie[categ][i].url, tab_categorie[categ][i].libelle, tab_categorie[categ][i].prix, tab_categorie[categ][i].texte, quantity, tabtitre[categ]);
        console.log(test);
    }
}
